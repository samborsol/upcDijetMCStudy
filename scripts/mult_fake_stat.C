...
void mult_fake_stat(){
	...
	TFile *fMCTrig = new TFile(treeFolder+"TrkCutsppreco_multMC_trigHLT_HIUPCSingleEG5NotHF2Pixel_SingleTrack_v1_jetCollectionak4PFJetAnalyzer_minJetPt0.root");
	...
	TH1F *h1 = new TH1F("h1","h1",6,-3,3);

	Int_t nEvents = dijetTree_MC->GetEntries();
	for(Long64_t i=0; i<nEvents; i++){
		dijetTree_MC->GetEntry(i);

		if( abs(djObj.eta1)>1.8 || abs(djObj.eta2)>1.8 || djObj.dphi<2 || djObj.pt1<20 || djObj.pt2<15 || djObj.mass<35) continue;

		h1->Fill(djObj.nJet-ngen);
	...
	c1->SetLogy();
        ...
        gStyle->SetStatY(0.6);
        h1->SetLineWidth(2);
        h1->GetXaxis()->SetTitle("(#Reco Jets)-(#Gen Jets)");
